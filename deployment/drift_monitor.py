...
# --- Funciones load_recent_data y load_reference_data (SIN CAMBIOS) ---
...
def load_reference_data(file_path, target_col):
    ...
    try:
        ...
        bool_cols = ['hascrcard', 'isactivemember', 'geography_france', 'geography_germany', 'geography_spain', 'gender_female', 'gender_male', 'numofproducts_1', 'numofproducts_2', 'numofproducts_3', 'numofproducts_4']
...
# --- Función generate_drift_report  ---
def generate_drift_report(df_current, df_reference, feature_columns, target_col, prediction_col, output_path_html):
...
if __name__ == "__main__":
    ...
    if not df_recent.empty: df_recent.columns = df_recent.columns.str.lower()
    if not df_hist.empty: df_hist.columns = df_hist.columns.str.lower()
    ...
    if not df_recent.empty and not df_hist.empty:
        ...
        if prediction_col_lower not in df_recent.columns: print(f"Error: Columna '{prediction_col_lower}' no encontrada.")
        elif target_col_lower not in df_hist.columns: print(f"Error: Columna '{target_col_lower}' no encontrada.")
        else:
            # Generar reporte y obtener resultados
            drift_results = generate_drift_report(
                 df_recent, df_hist, FEATURE_COLUMNS_TO_MONITOR,
                 target_col_lower, prediction_col_lower, # Nombres originales
                 OUTPUT_REPORT_PATH
             )
            ...
            if drift_results:
                try:
                    ...
                    # Comprobar si hubo drift (solo para log)
                    if drift_results["data_drift_detected"] or drift_results["target_drift_detected"]:
